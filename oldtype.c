...
const char *type_kind_to_string(TypeKind kind) {
    switch (kind) {
        case TY_NAMED: return "primitive";
...
uint64_t type_hash(const Type *t) {
    ...
    switch (t->kind) {
        case TY_NAMED: {
            ...
            uint64_t id = 0;
            if (t->u.primitive.intern_result && t->u.primitive.intern_result->entry) {
                id = (uint64_t)t->u.primitive.intern_result->entry->dense_index;
            } else if (t->u.primitive.intern_result) {
                /* Fallback to pointer if no dense index yet */
                id = (uint64_t)(uintptr_t)t->u.primitive.intern_result;
            }
            h = hash_combine(h, id);
            break;
        }
...
bool type_equals(const Type *a, const Type *b) {
    ...
    switch (a->kind) {
        case TY_NAMED:
            return a->u.primitive.intern_result == b->u.primitive.intern_result;
...
int type_cmp(const Type *a, const Type *b) {
    ...
    switch (a->kind) {
        case TY_NAMED: {
            uintptr_t A = (uintptr_t)a->u.primitive.intern_result;
            uintptr_t B = (uintptr_t)b->u.primitive.intern_result;
            if (A == B) return 0;
            return (A < B) ? -1 : 1;
        }
...
Type *resolve_ast_type(TypeChecker *tc, AstNode *ast_type) {
    ...
    switch (ast_ty->kind) {
        case AST_TYPE_PRIMITIVE: {
            if (!ast_ty->u.base.intern_result) return NULL;

            Type proto = {0};
            proto.kind = TY_NAMED;
            proto.u.primitive.intern_result = ast_ty->u.base.intern_result;
            ...
            if (ast_ty->u.array.size_expr) {
                ...
                if (ast_ty->u.array.size_expr->node_type == AST_LITERAL) {
                    ...
                    if (lit->type == INT_LITERAL) {
                        proto.u.array.size = lit->value.int_val;
...
int intern_function_types(TypeChecker *tc, Scope *scope, AstNode *program, TypeError *err) {
    ...
    Symbol *symbol_result = scope_define_symbol(scope, 
                           func_decl->data.function_declaration.intern_result, 
                           func_type, 
...
static void format_type_signature_into(char *buf, size_t buf_size, Type *t) {
    ...
    switch (t->kind) {
        case TY_NAMED: {
            if (t->u.primitive.intern_result && t->u.primitive.intern_result->key) {
                Slice *s = (Slice*)t->u.primitive.intern_result->key;
            ...
            snprintf(buf, buf_size, "<primitive>");
            return;
        }
        case TY_PTR: {
...
int intern_type_handles(TypeChecker *tc, DenseArenaInterner *keywords) {
    ...
    Type int32_type = { .kind = TY_NAMED, .u.primitive.intern_result = intern_peek(keywords, &int32_slice) };

    Slice int64_slice = { .ptr = "i64", .len = 3 };
    Type int64_type = { .kind = TY_NAMED, .u.primitive.intern_result = intern_peek(keywords, &int64_slice) };

    Slice float32_slice = { .ptr = "f32", .len = 3 };
    Type float32_type = { .kind = TY_NAMED, .u.primitive.intern_result = intern_peek(keywords, &float32_slice) };

    Slice float64_slice = { .ptr = "f64", .len = 3 };
    Type float64_type = { .kind = TY_NAMED, .u.primitive.intern_result = intern_peek(keywords, &float64_slice) };

    Slice bool_slice = { .ptr = "bool", .len = 4 };
    Type bool_type = { .kind = TY_NAMED, .u.primitive.intern_result = intern_peek(keywords, &bool_slice) };

    Slice char_slice = { .ptr = "char", .len = 4 };
    Type char_type = { .kind = TY_NAMED, .u.primitive.intern_result = intern_peek(keywords, &char_slice) };

    Slice string_slice = { .ptr = "str", .len = 3 };
    Type string_type = { .kind = TY_NAMED, .u.primitive.intern_result = intern_peek(keywords, &string_slice) };
...
int infer_binary_result_type(Type *left, Type *right, OpKind op, PromotionPlan *plan, TypeChecker *tc, TypeError *err) {
    ...
    /* Only handle primitive primitive types for arithmetic/promotions */
    ...
    int left_t  = left->u.primitive.intern_result && left->u.primitive.intern_result->entry ? left->u.primitive.intern_result->entry->dense_index : -1;
    int right_t = right->u.primitive.intern_result && right->u.primitive.intern_result->entry ? right->u.primitive.intern_result->entry->dense_index : -1;
...
int infer_unary_result_type(Type *operand, OpKind op, TypeChecker *tc, TypeError *err) {
    ...
    /* For primitive primitives we can use dense index directly; for composite types we will intern as needed. */
    int operand_dense_idx = -1;
    if (operand->kind == TY_NAMED && operand->u.primitive.intern_result && operand->u.primitive.intern_result->entry) {
        operand_dense_idx = operand->u.primitive.intern_result->entry->dense_index;
    } else {
